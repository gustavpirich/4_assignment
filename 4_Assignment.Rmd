...
We investigate four different cases of $\lambda_{1}$ and $\lambda_{2}$ to investigate the sensitivity of the IRFS, predictions and the autoregressive coefficients to different prior parameters. Case 1 starts with arbitrarily large values for $\lambda_{1}$ and $\lambda_{2}$, going down to case 4, where we set very small values for the hyperparameters. Note that we always keep the ordering that $\lambda_{1} < \lambda_{2}$, to account for the belief that the cross-variable lags are less informative than own variable lags. 
...
\textbf{Case 1: (\lambda_{1} = 3 and \lambda_{2} = 10)}
...
lambda1 <- 3; lambda2 <- 10; lambda3 <- 100
...
```
In case 1 the IRFs based on teh Cholesky decomposition resemble the typical behaviour we would expect. We focus on two particularly interesting IRFs, a monetary policy shock on inflation, unemployment. 

\textbf{Case 2: $(\lambda_{1} = 1$ and $\lambda_{2} = 3)$}
...
\textbf{Case 3: $\lambda_{1} = 0.1$, and $\lambda_{2} = 0.5$}
...
\textbf{Case 4: \lambda_{1} = 0.01, and \lambda_{2} = 0.1}

This case corresponds to a high precision for the variance matrix. We impose very agressive shrinkage on the variance and cross-variable lags.  
...
The higher the values for the hyperparamters, the more volatile the impulse response functions are. In case 4, were we impose extremely agressive shrinkage, the IRFs become straight lines. The estimated effect sizes are extremely small, to nonexistent. In case 4; a monetary policy shock has no effect on inflation in any period.  

Choosing smaller values for the $\lambda_{1}$, and controls the tightness of the random walk behaviour. If $\lambda_{1}$ is set to zero, then the VAR becomes a vector of univariate models. This accounts for the fact that in case 4, shocks only have a very small impact on the trajectory of other variables. If $\lambda_{1}$ goes to infinity, the data and conditional likelihood become more important.  

*Description of exercise where we only vary one parameter and leave the other lambda the same*

A suitable way to alleviate hyperparameter choices is to specify a prior distribution on the hyperparameters. One could specify a Gamma prior on plausible values for $\lambda_{1}$ and $\lambda_{2}$. Then one could estimate the models with the package BVAR. 
...
# cross lags:     (sig_i^2/sig_j^2)(lambda1 * lambda2/k)^2
# deterministics: lambda3*sig_i^2
lambda1 <- 0.5; lambda2 <- 1.5; lambda3 <- 100
V_prior <- array(0, c(k, k, M))
for(mm in 1:M){ # over all equations
  for(pp in 1:plag){ # over all lags
    for(kk in 1:M){ # over all coefficients
...
#changing the signs of the oil supply shock
dim(IRFchol_store)
