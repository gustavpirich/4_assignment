...
We investigate four different cases of $\lambda_{1}$ and $\lambda_{2}$ to investigate the sensitivity of the IRFs, predictions and the autoregressive coefficients to different prior parameters. Case 1 starts with extremely tight values for $\lambda_{1}$ and $\lambda_{2}$, up to case 3, where we set very high values for the hyperparamters. Note that we always keep the ordering that $\lambda_{1} < \lambda_{2}$, to account for the belief that the cross-variable lags are less informative than the own variable lags. 
...
```
In case one we impose very agressive shrinkage to the variance of the autoregressive coefficients.
...
In case 2, we use a plausible specification of 0.1 and 0.5. The IRFs show that the variables now influence each other more strongly, than in the case were we specified strong shrinkage. A monetary policy shock now has a clear positive impact on unemployement fading out after some months. 
...
```
In case three we impose very little shrinkage. The IRFs now display a more volatile behaviour than in case one and two, reflecting the fact that the data has received a higher relative importance in the estimation fo the posteriors. 

*Comparing IRFs based on Cholesky decomposition* \\
The higher the values for the hyperparamters, the more volatile the impulse response functions are. In case one, were we impose extremely agressive shrinkage, the IRFs become straight lines. The estimated effect sizes are extremely small, to nonexistent. 
In general, case two and three display very similar results.  

If $\lambda_{1}$ is set to zero, then the VAR becomes a vector of univariate models. This accounts for the fact that in case 1, shocks only have a minor impact on the trajectory of other variables. If $\lambda_{1}$ goes to infinity, the data and conditional likelihood receive an increasingly higher relative importance for the response functions.  

*Comparing IRFs based on Cholesky* \\
